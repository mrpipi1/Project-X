...
function addInput($column_data, $attribute = []){
    ...
    foreach($column_data as $col_num => $col_data) {
        ...
        $second_table_name = substr($input_name, 0, -3);    // wird beides erst später gebraucht, wenn ein select erstelln will zb bei den mitarbeitern welcher kurs unterrichtet wird.
